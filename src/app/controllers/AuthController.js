...
import User from '../models/users.js'; // Usar o modelo existente
...
class AuthController {
  async forgotPassword(req, res) {
    try {
      ...
      // Sempre retorna mensagem genérica para não vazar informações
      if (!user) {
      ...
      // Reseta tentativas se passou o tempo limite
      ...
      user.codigo_recuperacao_expiracao = new Date(now.getTime() + 5 * 60 * 1000); // 5 minutos
      await user.save();
...
Equipe Controle de Estoque`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
              ...
              <p>Olá,</p>
              <p>Recebemos uma solicitação para redefinir a senha da sua conta no <strong>Controle de Estoque</strong>.</p>
              ...
              <p style="font-size: 12px; color: #aaa;">Controle de Estoque © ${new Date().getFullYear()}</p>
            </div>
          `,
        });
      } catch (error) {
  ...
  async resetPassword(req, res) {
    try {
      ...
      user.codigo_recuperacao_expiracao = null;
      user.tentativas_recuperacao = 0; // Reseta tentativas após sucesso
      await user.save();
